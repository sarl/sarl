...
public class StandardSpawnService extends AbstractDependentService implements SpawnService {
	...
	@Override
	public UUID spawn(UUID spawningAgent, AgentContext parent, UUID agentID, Class<? extends Agent> agentClazz, Object... params) {
		if (isRunning()) {
			try {
	 ...
	 *
	 * @param spawningAgent the agent that has spawn the agent.
	 * @param context context in which the agent is spawn.
	 * @param spawnedAgent the spawn agent.
	 * @param initializationParameters list of the values to pass as initialization parameters.
	 */
	protected void fireAgentSpawned(UUID spawningAgent, AgentContext context, Agent spawnedAgent, Object[] initializationParameters) {
