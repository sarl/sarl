...
import io.sarl.sre.janus.network.boot.configs.SreNetworkConfigModule;
...
public class JanusLaunchNetworkTab extends JavaLaunchTab {
	...
	public Image getImage() {
		return JanusEclipsePlugin.getDefault().getImage("/icons/lan-network-icon.png"); //$NON-NLS-1$
	}
	...
	public void createControl(Composite parent) {
		...
		this.hazelcastMulticastGroup = SWTFactory.createGroup(this.globalGroup,
				MessageFormat.format("Node discovery ({0})", //$NON-NLS-1$
						CliUtilities.getCommandLineDefinition(
		...
		this.hazelcastIPMembersTextField.addModifyListener((event) -> scheduleUpdateJob());
