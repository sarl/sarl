...
import io.sarl.eclipse.tests.api.ProjectCreator;
import io.sarl.eclipse.tests.api.WorkbenchTestHelper;
...
public abstract class AbstractSarlMavenTest extends AbstractSarlUiTest {
	...
	protected IMarker[] createMavenProject(String pomFileContent) throws Exception {
		IFile pomFile = getBenchmarkHelper().createFileImpl(getBenchmarkHelper().getFullFileNameInProject("pom.xml"), pomFileContent);
		IProject project = getBenchmarkHelper().getProject();
	...
	protected void assertContainsMarker(String messagePart, IMarker... markers) {
		...
		assertionFailure().message("Missed marker: " + messagePart).expected(messagePart).actual(markerText.toString()).buildAndThrow();
	}
	...
	protected void assertNotContainsMarker(String messagePart, IMarker... markers) {
		...
		if (found) {
			assertionFailure().message("Unexpected marker: " + messagePart).expected(messagePart).actual(markerText.toString()).buildAndThrow();
		}
	}
	...
	public void assumeMavenRunning(IMarker[] markers) {
		assumeFalse(containsMavenErrorMarkers(markers));
	}

	@Override
	protected Module[] getInjectionModules() {
		final Module[] originalList = super.getInjectionModules();
		final Module[] newList = Arrays.copyOf(originalList, originalList.length + 1);
		newList[originalList.length] = (Binder binder) -> binder.bind(ProjectCreator.class).to(MavenProjectCreator.class);
		return newList;
	}
	 ...
	 * @mavenartifactid $ArtifactId$
	 */
	private class MavenProjectCreator implements ProjectCreator {
