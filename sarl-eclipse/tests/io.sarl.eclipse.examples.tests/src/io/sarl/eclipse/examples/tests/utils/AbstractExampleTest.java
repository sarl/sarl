...
import static io.sarl.eclipse.examples.tests.utils.ExamplesTestUtils.copySourceFiles;
import static io.sarl.eclipse.examples.tests.utils.ExamplesTestUtils.isMavenProject;
import static io.sarl.eclipse.examples.tests.utils.ExamplesTestUtils.preparePomFileForTest;
import static io.sarl.eclipse.examples.tests.utils.ExamplesTestUtils.unpackFiles;
	...
	protected static List<File> installFiles(ExampleDescription example, File projectRoot, boolean skipIfInvalidPom) throws Exception {
		...
		if (TestUtils.isEclipseRuntimeEnvironment()) {
			if (skipIfInvalidPom && isMavenProject(example.sourceFolder)) {
				abort("Pom file is not valid because Maven has not yet applied the macro replacements into the file");
			}
