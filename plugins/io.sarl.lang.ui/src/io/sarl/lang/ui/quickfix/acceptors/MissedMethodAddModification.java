		...
		for (String operationUriAsString : this.operationUris) {
			...
			if (overridden instanceof JvmOperation) {
				...
				if (!Utils.hasAnnotation(operation, DefaultValueUse.class)) {
					...
					IBlockExpressionBuilder blockBuilder = this.codeBuilderFactory.createXBlockExpression(
							container.eResource());
					appendable.append("// ") //$NON-NLS-1$
						.append(blockBuilder.getAutoGeneratedActionString());

					if (returnType != null && !returnType.isPrimitiveVoid()) {
						IExpressionBuilder exprBuilder = this.codeBuilderFactory.createXExpression(container.eResource());
