...
/**
 * Quick fixes for {@link IssueCodes#INVALID_EXTENDED_TYPE}.
 *
 ...
 */
public final class InvalidExtendedTypeModification extends SARLSemanticModification {
	...
	public static void accept(SARLQuickfixProvider provider, Issue issue, IssueResolutionAcceptor acceptor) {
		...
		if (data != null && data.length >= 2) {
			QualifiedName superTypeName = provider.qualifiedName(data[1]);
			String msg = MessageFormat.format(
					Messages.SARLQuickfixProvider_5,
					Messages.SARLQuickfixProvider_11, superTypeName.getLastSegment());
			InvalidExtendedTypeModification modification = new InvalidExtendedTypeModification();
		...
		if (!tools.removeToPreviousSeparator(issue, document, sep)) {
			if (!tools.removeToNextSeparator(issue, document, sep)) {
				tools.removeToPreviousKeyword(issue, document,
						tools.getGrammarAccess().getAgentAccess().getExtendsKeyword_3_0().getValue());
			}
		}
	}
