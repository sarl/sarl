 ...
 */
public final class InfixRedundantInterfaceModification extends SARLSemanticModification {
	...
	public static void accept(SARLQuickfixProvider provider, Issue issue, IssueResolutionAcceptor acceptor) {
		...
		if (data != null && data.length >= 2) {
			String mode = data[1];
			if (!"pre".equals(mode) && !"post".equals(mode)) { //$NON-NLS-1$//$NON-NLS-2$
				String redundantName = data[0];
				String msg = MessageFormat.format(Messages.SARLQuickfixProvider_6, redundantName);
				InfixRedundantInterfaceModification modification = new InfixRedundantInterfaceModification();
