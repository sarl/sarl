 ...
 */
public class CopyOfRuntimeEnvironmentTab extends JavaJRETab {
	...
	/**
	 */
	public CopyOfRuntimeEnvironmentTab() {
		//
	}
	...
	private class InstallChange extends SREInstallChangedAdapter {
		...
		public void sreRemoved(ISREInstall sre) {
			...
			if (index >= 0) {
				...
				CopyOfRuntimeEnvironmentTab.this.runtimeEnvironments.remove(index);
				CopyOfRuntimeEnvironmentTab.this.runtimeEnvironmentCombo.setItems(getSRELabels());
				selectSRE(selection);
			}
		}
		...
		public void sreAdded(ISREInstall sre) {
			if (isValid(sre, false)) {
				ISREInstall selection = getSelectedSRE();
				CopyOfRuntimeEnvironmentTab.this.runtimeEnvironments.add(sre);
				CopyOfRuntimeEnvironmentTab.this.runtimeEnvironmentCombo.setItems(getSRELabels());
				if (selection == null) {
					selectSRE(sre);
				} else {
