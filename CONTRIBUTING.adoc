...
=== 2.2. Structure of the SARL project

SARL project is divided into independent modules:

* `sarl-bom`: Contains the Maven Bills-of-Material, that permits to share the dependency configuration among all the SARL modules.
* `sarl-base-utils`: Several modules that are independent of SARL toolchain but used by this latter.
* `sarl-lang`: The definition of the SARL language and grammar, its compiler (Java batch version and Maven version).
* `sarl-sdk`: Set of libraries that compose the Standard Development Kit (SDK). The default SDK does not include all the libraries but only the key ones.
* `sarl-apputils`: Several generic modules for building command-line tools, mostly based on the Bootique library.
* `sarl-sre`: Definition of the SARL Runtime Environment (SRE), i.e., Janus platform.
* `sarl-docs`: Modules for generating the API document and the official document for SARL.
* `sarl-cli`: Command-line tools, e.g., `sarlc`, `sarldoc`, or `janus`.
* `sarl-eclipse`: The Eclipse environment for SARL.
* `sarl-officialdoc`: the official document of SARL, in Markdown format.

=== 2.3. First Compilation the SARL Source Code

Maven is the standard tool for compiling SARL.

We recommended to launch the Maven compilation process on the command line before importing the code in the Eclipse environment. Indeed, several code and resource files are updated by Maven with the correct configuration values.
...
Go into each module that are mentionned in section 2.2 (in the order provided in this section) and launch the maven command:
...
=== 2.4. Installing the Eclipse Development Environment
...
==== a) Contributions to the Eclipse tools of SARL
...
** Version: `2023-06` or higher (please ask to the developper's community on link:https://gitter.im/sarl/Lobby[Gitter]).
...
==== b) Contributions to the SARL tools excluding Eclipse
...
=== 2.4. Compilation of SARL before Submitting your Contribution
