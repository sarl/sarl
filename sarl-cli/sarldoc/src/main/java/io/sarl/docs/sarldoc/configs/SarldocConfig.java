...
public class SarldocConfig {
	...
	public static String findJavadocExecutable() {
		...
		if (!javadocExe.exists() || !javadocExe.isFile()) {
			// ----------------------------------------------------------------------
			// Try to find javadocExe from System.getProperty( "java.home" ) assuming
			// it is corresponding to the JDK_HOME
			// ----------------------------------------------------------------------
			javadocExe = FileSystem.join(new File(System.getProperty(JAVA_HOME_KEY)), BIN_FOLDER, javadocCommand);
