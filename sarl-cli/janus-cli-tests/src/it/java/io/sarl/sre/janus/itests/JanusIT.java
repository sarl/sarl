...
public class JanusIT {
	...
	public void runHelp() throws Exception {
		final String[] arguments = TestShell.mergeJarArguments(this.java.getAbsolutePath(), this.janusCmd.getAbsolutePath(), "--help");
	...
	public void runSREInJRE() throws Exception {
		final File tmpDir = FileSystem.createTempDirectory("janustest", null);
		FileSystem.deleteOnExit(tmpDir);
		final File currentDir = new File(FileSystem.CURRENT_DIRECTORY).getAbsoluteFile();
		final File sourceCode = FileSystem.join(currentDir, "src", "it", "resources", "test03.sarl");
		final File srcDir = FileSystem.join(tmpDir, "src", "main", "sarl");
		final File srcFile = FileSystem.join(srcDir, "test03.sarl");
		final File clsDir = FileSystem.join(tmpDir, "target", "classes");
		...
		// Run Janus SRE
		final String[] janusArguments = new String[] {
				"-cp",
