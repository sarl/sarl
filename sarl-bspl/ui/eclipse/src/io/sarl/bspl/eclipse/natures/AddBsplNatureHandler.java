...
/**
 * Action for converting the nature of a project to BSPL.
 *
 ...
 * @since 0.15
 */
public class AddBsplNatureHandler extends AbstractHandler {

	@Inject
	private BSPLProjectConfigurator configurator;
	...
	private boolean convertAllWithProgress(Shell shell, List<IProject> projects) throws ExecutionException {
		final IRunnableWithProgress op = new IRunnableWithProgress() {
			...
			public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
				monitor.beginTask(Messages.RemoveBsplNatureHandler_0, projects.size());
		...
		try {
			...
			throw new ExecutionException(
					MessageFormat.format(Messages.RemoveBsplNatureHandler_1, exception.getLocalizedMessage()), exception);
	...
	protected void doConvert(IProject project, IProgressMonitor monitor) throws ExecutionException {
		monitor.setTaskName(MessageFormat.format(Messages.AddBsplNatureHandler_0, project.getName()));
