...
* Replace `$Author: ID$` by the hyperlink of the contributor with the identifier `ID`.

=== 2.2) Remove snapshot markers

Remove all the occurrences of `-SNAPSHOT` in all the poms.

CAUTION: Do not remove the `-SNAPSHOT` in the Groovy scripts.

=== 2.3) Update the versions in the Eclipse configurations
...
=== 2.4) Comment any reference to the Snapshot Maven repository

=== 2.5) Update General and Contributor Readme Files
