...
@DisplayName("ecore: custom SarlField")
@Tag("core")
@Tag("unit")
public class SarlFieldImplCustomTest extends AbstractSarlTest {

	@Inject
	private SarlFactory factory;
	
	@Inject
	private SarlFieldImplCustom field;
	...
	@Test
	public void default_visibility_in_event() throws Exception {
		this.event.getMembers().add(this.field);
		assertEquals(JvmVisibility.PUBLIC, this.field.getVisibility());
	}
	...
	public void default_visibility_in_class() throws Exception {
		this.clazz.getMembers().add(this.field);
		assertEquals(JvmVisibility.PRIVATE, this.field.getVisibility());
	}
