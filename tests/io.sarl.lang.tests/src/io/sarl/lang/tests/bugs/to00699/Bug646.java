...
public class Bug646 extends AbstractSarlTest {
	...
	private final String EXPECTED = multilineString(
			...
			"      return false;",
			"    FrustumNodeIterator<T> other = (FrustumNodeIterator<T>) obj;",
			"    if (this.geometricFrustum == null) {",
			"      if (other.geometricFrustum != null)",
			"        return false;",
			"    } else if (!this.geometricFrustum.equals(other.geometricFrustum))",
			"      return false;",
			"    if (this.stack == null) {",
			"      if (other.stack != null)",
			"        return false;",
			...
			"    result = prime * result + ((this.geometricFrustum== null) ? 0 : this.geometricFrustum.hashCode());",
