...
public class Bug819Test extends AbstractSarlTest {
	...
	private static final String EXPECTED01 = multilineString2(false,
			...
			"    _$CAPACITY_USE$IO_SARL_API_CORE_LIFECYCLE$CALLER.spawn(Agent2.class, id, _$CAPACITY_USE$IO_SARL_API_CORE_DEFAULTCONTEXTINTERACTIONS$CALLER.getDefaultContext(), \"Gilbert\");",
			...
			"        UUID _uUID = it.getID();",
			"        return Objects.equals(_uUID, id);",
			"      }",
			"    }",
			...
			"      }",
			"    };",
			"    _$CAPACITY_USE$IO_SARL_API_CORE_DEFAULTCONTEXTINTERACTIONS$CALLER_1.emit(_hello, _function);",
			"  }",
			"",
			"  @Extension",
